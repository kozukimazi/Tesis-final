...
\insertimage[\label{img:fig1resultado}]{ejemplos/drawing2}{scale=0.80}{(a)Flujos de corriente de calor que libera el reservorio $\alpha$. (b)Flujo de corriente de partículas correspondiente a cada reservorio. Para obtener estos resultados se usaron las temperaturas $T=100$, $T_{d}=2$, razones de transición de $\gamma_{L}=\gamma^{U}_{R}= 1/100$, $\gamma_{R}= \gamma^{U}_{L}=1/600$, $\gamma_{D}=\gamma^{U}_{D}=1/50$ , parámetro de acoplamiento $g=5/1000$, interacciones de Coulomb $U=40$, $U_{F} = 500$ y por último niveles de energía $\epsilon=4$.}
...
\insertimage[\label{img:fig5resultado}]{ejemplos/concu1.png}{scale=0.45}{En líneas azul la norma-$l_{1}$ de la coherencia $\mathcal{C}_{l_{1}}$ y en líneas rojas la concurrencia $\mathcal{C}_{on}$.}
...
\insertimage[\label{img:fig6resultado}]{ejemplos/systemg.pdf}{scale=0.8}{Cantidades termodinámicas para el sistema $LR$ y el Demonio en función de $g/\gamma_{L}$, considerando $eV/T=1$ y el resto de parámetros corresponden a los de la Figura \ref{img:fig1resultado}. }

Se puede notar que a medida que $g\sim \gamma_{L}$ la potencia y la contribución de la información en el sistema $LR$ aumenta, lo cuál es interesante ya que permite ver como el acoplamiento favorece este hecho. Por otro lado, se puede ver las contribuciones individuales en los flujos de información y como este es influenciado debido al acoplamiento, es decir lo que se vee en la Figura \ref{img:fig7resultado}
...
Aquí sucede algo interesante, ya que cuando $g\sim \gamma_{L}$ las concurrencias y coherencias estan cerca de un peak, y a la vez los flujos de información del sitio $L$ y $R$ se vuelven $\dot{I}_{L} \approx \dot{I}_{R}$. Basicaménte a medida que el entrelazamiento y la coherencia actua en los dos sitios, los dos sitios entregan la misma información.
