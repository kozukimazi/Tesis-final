...
Inicialmente consideramos el caso en que las razones de transición no dependen de la frecuencia, es decir $\gamma_{i}(\omega) = \gamma_{i}$. Se estudiará el flujo de corriente ante una diferencia de potencial $\mu_{L}-\mu_{R} = eV$, considerando las razones de transición $\gamma_{L}=\gamma_{R}=1/100$, $\gamma_{d}=1/50$. Se obtiene

\insertimage[\label{img:transporte1}]{ejemplos/transprtcurrent.png}{scale=0.4}{Gráfico que representa la corriente de partículas en función de $eV/T$. Los parámetros son $E_{0}=4$, $E_{d}=\mu_{d}-U/2$, }
