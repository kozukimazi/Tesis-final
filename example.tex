...
Esto permite una expresión numérica para la función correlación $C^{\sigma}_{f}(t)$ y así evaluar su tiempo de correlación $\tau_{Bf}$. Para la otra función correlación se podrá encontrar el integral de manera analítica, ya que se puede escribir

\begin{align*}
        C^{\sigma}_{0}(t) & = \frac{\gamma_{0}}{2\pi} \int_{-\infty}^{\infty}d\omega e^{i\sigma \omega t} ( \exp(\sigma \beta(\omega - \mu))+ 1)^{-1}  \\
        & = \frac{\gamma_{0}}{2\pi} \int_{-\infty}^{\infty} d \omega e^{i\sigma \omega t} - \frac{\gamma_{0}}{2\pi} \int_{-\infty}^{\infty} d\omega \frac{e^{i \sigma \omega (t+\beta)} e^{-\sigma \beta \mu} }{ (\exp(\sigma \beta(\omega - \mu) ) + 1)  }  
\end{align*}
